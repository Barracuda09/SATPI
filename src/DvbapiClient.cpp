			...
			if (ptr[0] == 0x47) {
				...
				if (pid == 0) {
					...
					if (key != NULL) {
						...
						dvbcsa_decrypt(key, ptr + skip, 188 - skip);
...
//		const int cc            =   data[3] & 0x0f;
		...
		int len = sectionLength - 4 - 9 - prgLength; // 4 = CRC   9 = PMT Header from section length
	...
	for (;;) {
		...
		if (!_connected) {
			...
			if (retryTime < currTime) {
				if (initClientSocket(_client, 15011, inet_addr("192.168.0.109"))) {
					sendClientInfo();
